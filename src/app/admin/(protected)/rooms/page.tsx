...
import { STAY_DURATIONS, MAX_STAY_MONTHS, checkoutForDuration, type StayDurationKey } from "@/lib/stay"
...
// The board is colour-coded by gender at the BED level: pink = female,
// blue = male, slate = unrestricted/other. Occupancy *status* is conveyed by
// shape — a status dot, label and icon — so colour never has to carry two
// meanings at once (this also keeps it readable for colour-blind users).
type GenderKey = "male" | "female" | "neutral"

function bedGender(bed: Bed): GenderKey {
  return bed.genderRestriction === "female" ? "female"
    : bed.genderRestriction === "male" ? "male"
    : "neutral"
}

// Gender to colour a bed by. A vacant bed has no occupant, so we don't trust
// the Notion default (which falls back to "male"): it stays neutral unless a
// roommate in the same sharing room pins the gender.
function effectiveBedGender(bed: Bed, room: Room): GenderKey {
  if (bed.status !== "vacant") return bedGender(bed)
  if (room.type === "sharing") {
    const sibling = room.beds.find(b => b.bedNumber !== bed.bedNumber)
    const siblingSet = sibling && sibling.status !== "vacant" && sibling.status !== "blocked"
    if (siblingSet) {
      const sg = bedGender(sibling)
      if (sg !== "neutral") return sg
    }
  }
  return "neutral"
}

// A room reads as a single gender only when every bed shares it.
function roomGender(room: Room): GenderKey {
  const set = new Set(room.beds.map(b => effectiveBedGender(b, room)))
  return set.size === 1 ? [...set][0] : "neutral"
}

const genderTheme: Record<GenderKey, {
  label: string
  Icon: typeof Mars
  text: string
  accent: string    // solid rail / dot
  chip: string      // legend + room-summary pill
  occupied: string  // tile bg + border when filled
  vacant: string    // tile bg + border when empty (dashed)
  blocked: string   // tile bg + border when blocked
  ring: string      // focus ring colour
}> = {
  female: {
    label: "Female", Icon: Venus, text: "text-pink-600", accent: "bg-pink-500",
    chip: "bg-pink-100 text-pink-700 border-pink-200",
    occupied: "bg-pink-50 border-pink-200 hover:bg-pink-100/60",
    vacant:   "bg-background border-dashed border-pink-300 hover:bg-pink-50",
    blocked:  "bg-pink-50/50 border-pink-200",
    ring: "focus-visible:ring-pink-400",
  },
  male: {
    label: "Male", Icon: Mars, text: "text-sky-600", accent: "bg-sky-500",
    chip: "bg-sky-100 text-sky-700 border-sky-200",
    occupied: "bg-sky-50 border-sky-200 hover:bg-sky-100/60",
    vacant:   "bg-background border-dashed border-sky-300 hover:bg-sky-50",
    blocked:  "bg-sky-50/50 border-sky-200",
    ring: "focus-visible:ring-sky-400",
  },
  neutral: {
    label: "Any", Icon: Users, text: "text-slate-500", accent: "bg-slate-400",
    chip: "bg-slate-100 text-slate-600 border-slate-200",
    occupied: "bg-slate-50 border-slate-200 hover:bg-slate-100/60",
    vacant:   "bg-background border-dashed border-slate-300 hover:bg-slate-50",
    blocked:  "bg-slate-50/50 border-slate-200",
    ring: "focus-visible:ring-slate-400",
  },
}

// Status is reinforced by an icon (where one helps), never by tile colour.
const statusIcon: Partial<Record<BedStatus, typeof Lock>> = {
  blocked: Lock,
  special: Star,
}
...
function BedTile({ bed, gender, onClick }: { bed: Bed; gender: GenderKey; onClick: () => void }) {
  const g = genderTheme[gender]
  const st = statusConfig[bed.status]
  const isVacant = bed.status === "vacant"
  const isBlocked = bed.status === "blocked"
  const stateClass = isVacant ? g.vacant : isBlocked ? g.blocked : g.occupied
  const StatusIcon = statusIcon[bed.status]
  const GenderIcon = g.Icon

  return (
    <button
      onClick={onClick}
      aria-label={`Bed ${bed.bedNumber} · ${g.label} · ${st.label}${bed.guestName ? ` · ${bed.guestName}` : ""}`}
      className={`group/bed relative flex-1 min-w-0 overflow-hidden rounded-lg border ${stateClass} pl-3 pr-2 py-2 text-left transition-all duration-200 hover:shadow-sm focus:outline-none focus-visible:ring-2 ${g.ring} focus-visible:ring-offset-1 cursor-pointer ${isBlocked ? "opacity-80" : ""}`}
    >
      {/* Gender accent rail */}
      <span aria-hidden className={`absolute inset-y-1.5 left-0 w-1 rounded-full ${g.accent} ${isVacant ? "opacity-40" : ""}`} />

      {/* Status + gender row */}
      <div className="flex items-center gap-1 mb-1">
        <span aria-hidden className={`w-1.5 h-1.5 rounded-full ${st.dot} shrink-0`} />
        {StatusIcon && <StatusIcon className={`w-2.5 h-2.5 shrink-0 ${st.text}`} aria-hidden />}
        <span className={`text-[9px] font-semibold uppercase tracking-wide truncate ${st.text}`}>{st.label}</span>
        <GenderIcon className={`w-3 h-3 ml-auto shrink-0 ${g.text}`} aria-hidden />
      </div>

      {/* Guest / open slot */}
      <p className={`text-[11px] font-medium leading-tight truncate ${bed.guestName ? "text-foreground" : "text-muted-foreground"}`}>
        {bed.guestName ?? (isVacant ? "Open bed" : st.label)}
      </p>

      {/* Stay dates */}
      {bed.checkIn && (
        <p className="text-[9px] text-muted-foreground mt-1 flex items-center gap-0.5">
          <Calendar className="w-2 h-2 shrink-0" aria-hidden />
          <span className="truncate tabular-nums">
            {fmtDate(bed.checkIn)}{bed.checkOut ? ` → ${fmtDate(bed.checkOut)}` : " · open"}
          </span>
        </p>
      )}

      {/* Assign affordance on hover for vacant beds */}
      {isVacant && (
        <span className="mt-1 inline-flex items-center gap-0.5 text-[9px] font-semibold text-muted-foreground/70 opacity-0 group-hover/bed:opacity-100 transition-opacity">
          <UserPlus className="w-2.5 h-2.5" aria-hidden /> Assign
        </span>
      )}
    </button>
  )
...
function RoomCard({ room, onBedClick }: { room: Room; onBedClick: (r: Room, b: Bed) => void }) {
  ...
  const rg = genderTheme[roomGender(room)]
  const RoomGenderIcon = rg.Icon

  return (
    <div className="group bg-card border border-border rounded-xl overflow-hidden hover:border-slate-300 hover:shadow-md transition-all duration-200">
      {/* Header */}
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <span className="text-sm font-bold text-foreground leading-none">{room.number}</span>
              <RoomGenderIcon className={`w-3.5 h-3.5 shrink-0 ${rg.text}`} aria-label={`${rg.label} room`} />
            </div>
            <p className="text-[10px] text-muted-foreground leading-tight mt-0.5 truncate">
              {room.roomTier ?? (room.type === "sharing" ? "Sharing" : "Private")}
            </p>
          </div>
        </div>
        <span className={`shrink-0 text-[9px] font-medium px-1.5 py-0.5 rounded-full border ${cfg.badge}`}>{cfg.label}</span>
      </div>
      ...
      <div className="px-3 pb-3">
        <div className="flex gap-1.5">
          {room.beds.map(bed => (
            <BedTile key={bed.id} bed={bed} gender={effectiveBedGender(bed, room)} onClick={() => onBedClick(room, bed)} />
          ))}
        </div>

        {/* Footer: rate + flags */}
        {(room.monthlyRate > 0 || room.isBlocked || room.specialBookingType) && (
          <div className="mt-2.5 flex items-center justify-between gap-2">
            {room.monthlyRate > 0
              ? <span className="text-[10px] font-medium text-muted-foreground tabular-nums">₹{room.monthlyRate.toLocaleString("en-IN")}/mo</span>
              : <span />}
            {room.isBlocked && (
              <span className="text-[10px] text-red-500 flex items-center gap-0.5 min-w-0">
                <ShieldAlert className="w-2.5 h-2.5 shrink-0" /> <span className="truncate">{room.blockReason?.split("—")[0].trim()}</span>
              </span>
            )}
            {room.specialBookingType && (
              <Badge variant="outline" className="shrink-0 text-[9px] h-4 px-1.5 bg-indigo-50 text-indigo-700 border-indigo-200 capitalize">
...
function BedModal({
  ...
  const [ciDuration, setCiDuration]   = useState<StayDurationKey>("1m")
  const ciCheckOut = checkoutForDuration(ciDate, ciDuration)
        ...
        {mode === "checkin" && (
          <div className="space-y-3 pt-1">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="space-y-1">
                <label className="text-[11px] text-muted-foreground">Stay Duration *</label>
                <Select value={ciDuration} onValueChange={v => setCiDuration(v as StayDurationKey)}>
                  <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="col-span-2">
                <p className="text-[11px] text-muted-foreground flex items-center gap-1.5">
                  ...
                  <span className="font-medium text-foreground tabular-nums">
                    {ciCheckOut ? formatDateLong(ciCheckOut) : "—"}
                  </span>
                  <span className="text-muted-foreground/60">· max {MAX_STAY_MONTHS} months</span>
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5 pt-1 border-t border-border">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-[11px] text-muted-foreground">Create monthly rent subscription (auto-debits from 2 days before each month begins)</span>
              </label>
            </div>
...
export default function RoomsPage() {
  ...
  const counts = {
    ...
    incoming: allBeds.filter(b => b.status === "incoming").length,
    blocked:  scopedRooms.filter(r => r.isBlocked).length,
    special:  scopedRooms.filter(r => r.specialBookingType).length,
  }
  const effGenders = scopedRooms.flatMap(r => r.beds.map(b => effectiveBedGender(b, r)))
  const genderCounts: Record<Exclude<GenderKey, "neutral">, number> = {
    female: effGenders.filter(g => g === "female").length,
    male:   effGenders.filter(g => g === "male").length,
  }
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-border bg-card px-3.5 py-3 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        {/* Gender colour key — the board's primary colour system */}
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mr-0.5">Gender key</span>
          {(["female", "male", "neutral"] as GenderKey[]).map(k => {
            const g = genderTheme[k]
            const GenderIcon = g.Icon
            const n = k === "neutral" ? undefined : genderCounts[k]
            return (
              <span key={k} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[11px] font-medium ${g.chip}`}>
                <GenderIcon className="w-3 h-3" aria-hidden /> {g.label}
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {(Object.entries(counts) as [BedStatus, number][]).map(([status, count]) => {
            ...
            return (
              <div key={status} className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border border-border bg-background text-[11px] font-medium text-muted-foreground">
                <span className={`w-1.5 h-1.5 rounded-full ${cfg.dot}`} aria-hidden />
                <span className="tabular-nums text-foreground">{count}</span> {cfg.label}
              </div>
            )
          })}
        </div>
