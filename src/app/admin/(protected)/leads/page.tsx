...
// Co-living and residency are separate revenue streams feeding one backend;
// the board can be viewed unified or per stream for financial trade-offs.
const STREAMS: { id: "all" | Lead["leadType"]; label: string }[] = [
  { id: "all",        label: "All" },
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        <Badge
          variant="outline"
          className={`text-[10px] h-4 px-1.5 ${lead.leadType === "residency" ? "border-purple-200 text-purple-700 bg-purple-50" : "border-sky-200 text-sky-700 bg-sky-50"}`}
        >
          {lead.leadType === "residency" ? "Residency" : "Co-living"}
        </Badge>
        {lead.property && (
          <Badge variant="outline" className="text-[10px] h-4 px-1.5 text-muted-foreground">
            {lead.property === "safina-plaza" ? "Plaza" : "—"}
          </Badge>
        )}
        {lead.roomType && (
...
export default function LeadsPage() {
  ...
  const [stream, setStream]     = useState<"all" | Lead["leadType"]>("all")
  ...
  const propertyScoped = scope === "all" ? leads : leads.filter(l => l.property === scope || l.property === null)
  const scopedLeads = stream === "all" ? propertyScoped : propertyScoped.filter(l => l.leadType === stream)
  ...
  async function updateType(lead: Lead, leadType: Lead["leadType"]) {
    setUpdating(true)
    await fetch("/api/leads/update", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ notionPageId: lead.notionPageId, leadType }),
    })
    setUpdating(false)
    setSelected(null)
    fetchLeads()
  ...
  return (
    <div className="space-y-4">
      {/* Stream split: co-living vs residency */}
        ...
        {STREAMS.map(s => (
          <button
            ...
            className={`px-3 h-7 rounded-md text-[11px] font-medium transition-colors ${
              stream === s.id ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
            }`}
          >
            {s.label}
            <span className="ml-1.5 text-[10px] text-muted-foreground">
              {s.id === "all" ? propertyScoped.length : propertyScoped.filter(l => l.leadType === s.id).length}
            </span>
          </button>
        ))}
      </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2.5 pt-1">
                {[
                  ...
                  ["Stream",   selected.leadType === "residency" ? "Residency" : "Co-living"],
                  ["Property", selected.property === "safina-plaza" ? "Safina Plaza" : "—"],
              ...
              <Button
                size="sm"
                variant="ghost"
                className="h-7 text-[11px] text-muted-foreground -mt-1"
                disabled={updating}
                onClick={() => updateType(selected, selected.leadType === "residency" ? "co-living" : "residency")}
              >
                Move to {selected.leadType === "residency" ? "Co-living" : "Residency"} →
              </Button>
